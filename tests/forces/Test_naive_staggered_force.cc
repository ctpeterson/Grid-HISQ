    ...
    Source file: ./tests/Test_naive_staggered_force.cc
...
int main (int argc, char ** argv)
{
  ...
  LatticeStaggeredFermion phi        (&Grid); gaussian(pRNG,phi);
  LatticeStaggeredFermion Mphi       (&Grid); 
  LatticeStaggeredFermion MphiPrime  (&Grid); 
  ...
  Ds.MDeriv(tmp , Mphi,  phi,DaggerNo );  UdSdU=tmp;
  Ds.MDeriv(tmp , phi,  Mphi,DaggerYes ); UdSdU=(UdSdU+tmp);

  // Take the trace
  UdSdU = Ta(UdSdU);
  
  LatticeStaggeredFermion Ftmp      (&Grid);
